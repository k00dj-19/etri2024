...
class Transformer(nn.Module):
    ...
    # self->cross->self
    # 첫번째 각자 self attention 이후에 concat 하여 text와 cross attention
    def forward(self, src, src_a, mask, mask_a, query_embed, pos_embed, pos_embed_a, video_length=None):
        ...
        #print("src",src.shape)
        # cross attention 이전에 video 부분만 self attention 통과
        src_only_video = src[:video_length + 1]  # (L, batch_size, d)
        mask_only_video = mask[:, :video_length + 1]  # (batch_size, L) 
        pos_embed_only_video = pos_embed[:video_length + 1]  # (L, batch_size, d)
        src_only_video = self.encoder(src_only_video, src_key_padding_mask=mask_only_video, pos=pos_embed_only_video)  # (L, batch_size, d)
        #print("src_only_video",src_only_video.shape)
        # # 다시 src에 넣어줌
        # src = torch.cat((src_only_video, src[video_length + 1:]), dim=0)  # (L, batch_size, d)
        # mask = torch.cat((mask_only_video, mask[:, video_length + 1:]), dim=1)  # (batch_size, L)
        # pos_embed = torch.cat((pos_embed_only_video, pos_embed[video_length + 1:]), dim=0)  # (L, batch_size, d)
        #print("video_length:",video_length)
        if src_a is not None:
            src_a = src_a.permute(1, 0, 2)  # (L, batch_size, d)
            pos_embed_a = pos_embed_a.permute(1, 0, 2)  # (L, batch_size, d)
        
            # cross attention 이전에 self attention 통과
            src_only_audio = src_a[:video_length + 1]  # (L, batch_size, d)
            mask_only_audio = mask_a[:, :video_length + 1]  # (batch_size, L)
            pos_embed_only_audio = pos_embed_a[:video_length + 1]  # (L, batch_size, d)
            src_only_audio = self.encoder(src_only_audio, src_key_padding_mask=mask_only_audio, pos=pos_embed_only_audio)  # (L, batch_size, d)
            #print("src_only_audio",src_only_audio.shape)
            # 다시 src_a에 넣어줌
            # src_a = torch.cat((src_only_audio, src_a[video_length + 1:]), dim=0)  # (L, batch_size, d)
            # mask_a = torch.cat((mask_only_audio, mask_a[:, video_length + 1:]), dim=1)  # (batch_size, L) 
            # pos_embed_a = torch.cat((pos_embed_only_audio, pos_embed_a[video_length + 1:]), dim=0)  # (L, batch_size, d)

            # concat src, src_a
            src_va = torch.cat((src_only_video, src_only_audio), dim=0)  # (L, batch_size, 2d) torch.Size([152, 32, 256])   
            mask_va = torch.cat((mask_only_video, mask_only_audio), dim=1)  # (batch_size, L) torch.Size([32, 152])
            pos_embed_va = torch.cat((pos_embed_only_video, pos_embed_only_audio), dim=0)  # (L, batch_size, d)                                             
            # print('src_va : ', src_va.shape)
            # print("mask_va : ", mask_va.shape)
            # print("pos_embed_va : ", pos_embed_va.shape)
            src = torch.cat((src_va, src[video_length + 1:]), dim=0)  # (L, batch_size, 2d)
            mask = torch.cat((mask_va, mask[:, video_length + 1:]), dim=1)  # (batch_size, L)
            pos_embed = torch.cat((pos_embed_va, pos_embed[video_length + 1:]), dim=0)  # (L, batch_size, d)
            video_length = video_length*2
            # print('src : ', src.shape)
            # print("mask : ", mask.shape)
            # print("pos_embed : ", pos_embed.shape)
            # mask와 mask_a의 모든 요소가 같은지 확인 => 같다.
            # print('mask == mask_a : ', (mask == mask_a).all())
            # # pos_embed와 pos_embed_a의 모든 요소가 같은지 확인 => 같다.
            # print('pos_embed == pos_embed_a : ', (pos_embed == pos_embed_a).all()) 
            #src = torch.cat((src, src_a), dim=2)  # (L, batch_size, 2d)
        # transformer cross-attention encoder 통과
        ...
        #     pos_embed_a = torch.cat((pos_embed_only_audio, pos_embed_a[video_length + 1:]), dim=0)  # (L, batch_size, d)
        #     # cross-attntion 통과
        #     src_a = self.t2v_encoder(src_a, src_key_padding_mask=mask_a, pos=pos_embed_a, video_length=video_length)  # (L, batch_size, d)
...
class T2V_TransformerEncoderLayer(nn.Module):
    ...
    def forward_post(self,
        ...
        #print("after self_attention:", src2.shape)
