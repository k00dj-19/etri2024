...
class ClipFeatureExtractor:
    ...
    @torch.no_grad()
    def encode_text_each(self, text):
        encoded_texts = self.tokenizer(text, context_length=77).to(self.device)
        output = self.clip_extractor.encode_text(encoded_texts)
        valid_lengths = (encoded_texts != 0).sum(1).tolist()
        #print('valid_lengths:', valid_lengths)
        batch_last_hidden_states = output["last_hidden_state"]
        text_features = batch_last_hidden_states[0, :valid_lengths[0]]
        #print('text_features:', text_features.shape)
        return text_features  # (L, d) torch tensor
